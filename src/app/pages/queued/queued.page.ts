...
@Component({
  selector: 'app-queued',
  templateUrl: './queued.page.html',
  styleUrls: ['./queued.page.scss'],
  standalone:false
})
export class QueuedPage implements OnInit {
  ...
  async functionReturnData(){
    ...
    this.chatService.chatGetData(sendValues).then(async data=>{
      ...
      if(errorData == 1){
        ...
        if(typeof this.returnResultData.data.queue!== 'undefined'){
          let counter = 0;
          let countOfData = 0;
          this.returnArrayChatFromServer = this.returnResultData.data.queue;
          Object.keys(this.returnArrayChatFromServer).forEach(key => {
             this.returnChatArray[counter]=[];
              ...
              this.returnChatArrayQue.add(this.returnChatArray[counter]['mobile']);
              counter++;
          });
      ...
      this.returnResultDataQueSizeAll = 0;
        if(errorData == 1){
          if(typeof this.returnResultData.data.process!== 'undefined'){
            this.returnArrayChatFromServerQue = this.returnResultDataQue.data.process;
            Object.keys(this.returnArrayChatFromServerQue).forEach(key => {
                  this.returnChatArrayAll.add(this.returnArrayChatFromServerQue[key].mobile); 
            });
        }
      }
      this.returnResultDataQueSizeAll = this.returnChatArrayAll.size;
      this.timeCheck = setTimeout(() => {
        this.functionReturnDataAll();
      }, 3000);
    }).catch(error=>{
      this.functionReturnDataAll();
    });
 ...
 functionSearch(event:any){
  ...
  functionChats(number:any,chatSessionId:any,userName:any,name:any){
    this.navCtrl.navigateRoot(['/chats', {number:number,chatSessionId:chatSessionId,userNameUsed:userName,backUrl:1,name:name}]);
  }
